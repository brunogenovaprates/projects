...
def prepare(args, motd):
    ...
    try:
        ...
        print("Invalid option: '{}'. Must be a number: 1 for 1st option, 2 for 2nd, ...".format(
            args.option))
        exit(1)
    ...
    if (args.p == True):
        try:
            ...
            page = requests.get("https://proxy-daily.com",
    ...
    while True:
        ...
        thr = threading.Thread(target=vote, args=(
            url, checkboxID, headers, proxy))
        thr.daemon = True
        thr.start()
...
def vote(url, checkboxID, headers, proxy=None):
    ...
    try:
        ...
        page = requests.post(url, data={"security-token": secToken1, secToken2: "",
                                        "options": checkboxID}, headers=headers, proxies=proxies, timeout=10).text
